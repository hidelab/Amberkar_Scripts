...
setwd("./MAYO/TCX/results_dC/")
tcx_all_counts=data.frame(cbind(tcx_c_counts,tcx_t_counts),stringsAsFactors = F)
AD_dCP=dCP(n_genes =55,all_exprs = tcx_all_counts,c_exprs = tcx_c_counts,t_exprs = tcx_t_counts,blocksize = 10)
names(AD_dCP)=rownames(tcx_all_counts)[1:55]
saveRDS(AD_dCP,"mayo_AD_dCP.RDS")
system("awk 'FNR==1 && NR!=1 { while (/^<header>/) getline; }    1 {print}' tmp_scc_CorrC*.txt >mayo_Control_dCL_allResults.txt")
system("awk 'FNR==1 && NR!=1 { while (/^<header>/) getline; }    1 {print}' tmp_scc_CorrT*.txt >mayo_AD_dCL_allResults.txt")
mayo_tcx_control_dCL=fread("mayo_Control_dCL_allResults.txt",sep = "\t",header = T,showProgress = T,data.table = F)
mayo_tcx_AD_dCL=fread("mayo_AD_dCL_allResults.txt",sep = "\t",header = T,showProgress = T,data.table = F)
mayo_tcx_control_dCL.graph=graph.data.frame(d=mayo_tcx_control_dCL[,c(1:2)],directed = F)
mayo_tcx_AD_dCL.graph=graph.data.frame(d=mayo_tcx_AD_dCL[,c(1:2)],directed = F)
...
random_dCP_exprList=replicate(n = permut,Random_dCP(n_genes = 55,all_exprs = tcx_all_counts,c_exprs = c_counts,t_exprs = t_counts,blocksize = 10))
colnames(random_dCP_exprList)=paste("Perm",1:permut,sep = "")
rownames(random_dCP_exprList)=rownames(all_counts)[1:55]
