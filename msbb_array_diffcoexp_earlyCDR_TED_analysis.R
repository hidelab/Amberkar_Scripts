...
msbb_array_earlyCDR_DRrank.TED=msbb_array_earlyCDR_DRrank.TED=vector(mode = "list",length = 19)
names(msbb_array_earlyCDR_DRrank.TED)=names(msbb_array_earlyCDR_DRrank.TED)=names(msbb_gse84422_early_diffcoexp_results)
for(i in c(1,10)){
   msbb_array_earlyCDR_DRrank.TED[[i]]=DRrank(DCGs=msbb_gse84422_earlyCDR.DCGs[[i]],DCLs=msbb_gse84422_earlyCDR.DCLs[[i]],tf2target=regnet_tf2target.HGNC,expGenes=rownames(msbb_gse84422_GPL570_samplesToAnalyse.exprs$Amygdala),rank.method="TED",Nperm=1000)
   saveRDS(msbb_array_earlyCDR_DRrank.TED[[i]],paste(names(msbb_array_earlyCDR_DRrank.TED)[i],"earlyCDR_DRrank_TED.RDS",sep = "_"))
}
