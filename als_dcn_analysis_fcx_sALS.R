...
setwd("/Users/sandeepamberkar/Work/Data/ALS_Claire_Datasets/FCX_ALS_C9orf72_sALS//")
...
fcx_als_gene_counts=data.frame(fcx_als_gene_counts_rawData[,c(28,grep(pattern = "sALS",x = colnames(fcx_als_gene_counts_rawData)),grep(pattern = "control",x = colnames(fcx_als_gene_counts_rawData)))])
fcx_als_gene_counts.Control=grep(pattern = "control",x = colnames(fcx_als_gene_counts_rawData),value = T)
fcx_als_gene_counts.sALS=grep(pattern = "sALS",x = colnames(fcx_als_gene_counts_rawData),value = T)

fcx_als_gene_counts.colData=data.frame(condition=c(rep("sALS",length(fcx_als_gene_counts.sALS)),rep("Control",length(fcx_als_gene_counts.Control))),
                                                   type="paired-end",row.names=colnames(fcx_als_gene_counts)[-1])
