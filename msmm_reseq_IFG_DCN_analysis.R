...
msmm_reseq_design_matrix=read.table("./MSMM/MSSM_FP_STG_PHG_IFG_Design.tsv",sep = "\t",header = T,as.is = T)
number_of_combinations=choose(nrow(msmm_data),2)
c_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisIFG.CONTROL==1])]
t_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisIFG.AD==1])]
...
while(start < number_of_combinations){
  ...
  fwrite(result, file=paste("./MSMM/results_IFG/mssm_reseq_IFG_cocor_tmp", i, ".txt",sep = ""), sep="\t",col.names = T,row.names = F,buffMB = 100,nThread = 16,quote = F)
  i<-i+1
  start<-i*blocksize+1
  end<-min((i+1)*blocksize, number_of_combinations)
}
