...
AD_dCP=dCP(n_genes =length(rownames(all_counts)),all_exprs = all_counts,c_exprs = c_counts,t_exprs = t_counts,blocksize = 10)
saveRDS(AD_dCP,"ROSMAP_AD_dCP.RDS")
system("awk 'FNR==1 && NR!=1 { while (/^<header>/) getline; }    1 {print}' tmp_CorrC*.txt >ROSMAP_Control_dCL_allResults.txt")
system("awk 'FNR==1 && NR!=1 { while (/^<header>/) getline; }    1 {print}' tmp_CorrT*.txt >ROSMAP_AD_dCL_allResults.txt")


#Initialise permutation matrix as list
random_dCG_exprList=lapply(AD_dCP,function(x){x<-c(rep(0,1000));x})
dir.create("./ROSMAP/results_random_dC/",mode = 0777)
setwd("./ROSMAP/results_random_dC/")
random_dCP_exprList=Random_dCP(n_genes = length(rownames(all_counts)),all_exprs = all_counts,c_exprs = c_counts,t_exprs = t_counts,blocksize = 10)
