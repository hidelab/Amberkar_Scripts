...
msbb_stg_c_counts=msbb_reseq_data2.agg[,msbb_covariates$SampleID[msbb_covariates$BrainRegion.Diagnosis=="STG.CONTROL"]]
msbb_stg_t_counts=msbb_reseq_data2.agg[,msbb_covariates$SampleID[msbb_covariates$BrainRegion.Diagnosis=="STG.AD"]]

msbb_stg_DCe=DCe(exprs.1 = msbb_stg_c_counts,exprs.2 = msbb_stg_t_counts,r.method = "spearman",p = 0.05,link.method = "qth",cutoff = 0.05)
#msbb_stg_DCsum.res=DCsum(msbb_stg_DCp,msbb_stg_DCe,DCpcutoff=0.05,DCecutoff=0.05)
DCecutoff = 0.25
msbb_stg_DCe.DCG <- msbb_stg_DCe$DCGs[msbb_stg_DCe$DCGs[, "q"] < DCecutoff, ]
msbb_stg_DCe.DCG <- data.frame(DCG = rownames(msbb_stg_DCe.DCG), msbb_stg_DCe.DCG)
DCG <-msbb_stg_DCe.DCG
x<-msbb_stg_DCe$DCLs
x<-subset(x, subset=(Gene.1 %in% rownames(DCG) | Gene.2 %in% rownames(DCG) ))
expGenes<-rownames(msbb_stg_DCe$DCGs)
msbb_stg_DCe_DRsort.res<- DRsort(DCG, x, regnet_tf2target, expGenes)
saveRDS(msbb_stg_DCe_DRsort.res,"MSBB_STG_DCe_DRsort.res.RDS")
saveRDS(msbb_stg_DCe,"MSBB_STG_DCe_res.RDS")
