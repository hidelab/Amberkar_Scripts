...
c_exprs_rank=msmm_data[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisFP.CONTROL==1])]
t_exprs_rank=msmm_data[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisFP.AD==1])]
...
while(start < number_of_combinations){
  ...
  fwrite(result, file=paste("results_FP/mssm_reseq_FP_cocor_tmp", i, ".txt",sep = ""), sep="\t",col.names = T,row.names = F,buffMB = 100,nThread = 16,quote = F)
  i<-i+1
  start<-i*blocksize+1
  end<-min((i+1)*blocksize, number_of_combinations)
}
...
fwrite(allResults_FDR,"msmm_reseq_FP_allResults_DiffCorr_FDR.txt",sep = "\t",col.names = T,row.names = F,buffMB = 100,nThread = 12,showProgress = T)
...
saveRDS(control_DCN,file = paste("mssm_reseq_FP_Control.RDS",sep = ""))
saveRDS(AD_DCN,file = paste("mssm_reseq_FP_AD.RDS",sep = ""))
