...
for(i in c(1,10)){
  msbb_array_DRrank.TED[[i]]=DRrank(DCGs=msbb_gse84422.DCGs[[i]],DCLs=msbb_gse84422.DCLs[[i]],tf2target=regnet_tf2target.HGNC,expGenes=rownames(msbb_gse84422_GPL570_samplesToAnalyse.exprs$Amygdala),rank.method="TED",Nperm=100)%>%rownames_to_column("Gene")
  saveRDS(msbb_array_DRrank.TED[[i]],paste(names(msbb_array_DRrank.TED)[i],"DRrank_TED.RDS",sep = "_"))
}
for(i in c(4,16:19)){
  msbb_array_DRrank.TED[[i]]=DRrank(DCGs=msbb_gse84422.DCGs[[i]],DCLs=msbb_gse84422.DCLs[[i]],tf2target=regnet_tf2target.HGNC,expGenes=rownames(msbb_gse84422_GPL96_97_samplesToAnalyse.exprs$`Frontal Pole`),rank.method="TED",Nperm=100)%>%rownames_to_column("Gene")
  saveRDS(msbb_array_DRrank.TED[[i]],paste(names(msbb_array_DRrank.TED)[i],"DRrank_TED.RDS",sep = "_"))
}
