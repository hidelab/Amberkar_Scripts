...
c_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisFP.CONTROL==1])]#16035
t_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisFP.AD==1])]#12000
n.c<-ncol(c_exprs_rank)
n.t<-ncol(t_exprs_rank)
gene.names<-rownames(msmm_data2)
...
write.table(allResults_FDR,"msmm_reseq_FP_allResults_DiffCorr_FDR.txt",sep = "\t",col.names = T,row.names = F)
