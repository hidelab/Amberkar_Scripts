...
#  msbb_gse84422.pData=lapply(msbb_gse84422.pData,function(x){x$SampleTypeCDR[which(x$`clinical dementia rating:ch`>=3)]="CDR345";x})
# 
...
#  msbb_gse84422.pData=lapply(msbb_gse84422.pData,function(x){x$SampleType[which(x$SampleTypeCDR=="CDR345"&(x$`neuropathological category:ch1`!='Normal'&x$`neuropathological category:ch1`!='Possible AD'))]="AD";x})
...
#Diffcoexp analysis
for(i in length(names(msbb_gse84422_GPL96_97_samplesToAnalyse)):1){
  ...
  saveRDS(diffcoexp_out,paste(names(msbb_gse84422_GPL96_97_samplesToAnalyse.exprs)[i],"LateAD_diffcoexp/lateAD_diffcoexp_results.RDS",sep = "_"))
  proc.time()
}
