...
"linkFilter"<-function(exprs.1, exprs.2, rth=0.5, qth=0.1, r.method=c('pearson','spearman')[1], q.method=c("BH","holm", "hochberg", "hommel", "bonferroni", "BY","fdr")[1]) {
    if(!all(rownames(exprs.1)==rownames(exprs.2))) {
        stop("rownames of two expression matrices must be the same!")
    }
...
"diffcoexp" <-
function(exprs.1, exprs.2, rth=0.5, qth=0.1, r.diffth=0.5, q.diffth=0.1, r.method=c('pearson','spearman')[1], q.method=c("BH","holm", "hochberg", "hommel", "bonferroni", "BY","fdr")[1]){
    if(!all(rownames(exprs.1)==rownames(exprs.2))) {
        stop("rownames of two expression matrices must be the same!")
    }
	...
	cor.filtered = linkFilter(exprs.1, exprs.2, r.method=r.method, rth=rth, qth=qth)
	if(!is.null(cor.filtered)) {
		print("Finished running linkFilter.")
	}
    ...
    if ( nrow(cor.filtered)==0 ) {
        name.all = NULL
        stop("No gene pairs passed half-threshholding!")
    } else {
...
################################################################################################
    ...
    if (n.DCL == 0) {
        stop("No DCL meets the thresholds!")
    } else {
