...
msmm_reseq_design_matrix=read.table("./MSMM/MSSM_FP_STG_PHG_IFG_Design.tsv",sep = "\t",header = T,as.is = T)
number_of_combinations=choose(nrow(msmm_data),2)
c_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisPHG.CONTROL==1])]
t_exprs_rank=msmm_data2[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisPHG.AD==1])]
...
fwrite(allResults_FDR,"msmm_reseq_PHG_allResults_DiffCorr_FDR.txt",sep = "\t",col.names = T,row.names = F,buffMB = 100,nThread = 12,showProgress = T)
...
saveRDS(control_DCN,file = paste("mssm_reseq_PHG_Control.RDS",sep = ""))
saveRDS(AD_DCN,file = paste("mssm_reseq_PHG_AD.RDS",sep = ""))
