...
c_exprs_rank=msmm_data2.agg[,which(colnames(msmm_data2.agg)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisPHG.CONTROL==1])]
t_exprs_rank=msmm_data2.agg[,which(colnames(msmm_data2.agg)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisPHG.AD==1])]
...
while(start < number_of_combinations){
  ...
  fwrite(result, file=paste("./MSMM/results_PHG/mssm_reseq_PHG_cocor_corrPval_tmp", i, ".txt",sep = ""), sep="\t",col.names = T,row.names = F,buffMB = 100,nThread = 16,quote = F)
  i<-i+1
  start<-i*blocksize+1
  end<-min((i+1)*blocksize, number_of_combinations)
}
...
system(paste("find . -name '*.txt'|grep 'tmp'|xargs -n 1 tail -n +2 ",paste(">MSMM_ReSeq_PHG_corrPval",sep = "_",collapse = "_"),"_allResults_DiffCorr.txt",sep = ""))
...
fwrite(allResults_FDR,"msmm_reseq_PHG_corrPval_allResults_DiffCorr_FDR.txt",sep = "\t",col.names = T,row.names = F,buffMB = 100,nThread = 12,showProgress = T)
...
saveRDS(control_DCN,file = paste("mssm_reseq_PHG_corrPval_Control.RDS",sep = ""))
saveRDS(AD_DCN,file = paste("mssm_reseq_PHG_corrPval_AD.RDS",sep = ""))
