...
fwrite(allResults_FDR,"IPAH_corrPval_allResults_DiffCorr_FDR.txt",sep = "\t",col.names = T,row.names = F,buffMB = 200,nThread = 18)
HV_DCN=IPAH_DCN=graph.data.frame(d = allResults_FDR[which(allResults_FDR$FDR.cocor<=0.1),c(1:2)],directed = F)
E(HV_DCN)$weight=allResults_FDR[which(allResults_FDR$FDR.cocor<=0.1),3]+1
E(IPAH_DCN)$weight=allResults_FDR[which(allResults_FDR$FDR.cocor<=0.1),6]+1
saveRDS(HV_DCN,file = paste("HV_DCN.RDS",sep = ""))
saveRDS(IPAH_DCN,file = paste("IPAH_DCN.RDS",sep = ""))
