...
msbb_array_DRrank.TDD[c(1,10)]=mcmapply(FUN=function(a,b)DRrank(DCGs = a,DCLs = b,tf2target = regnet_tf2target.HGNC,expGenes = rownames(msbb_gse84422_GPL570_samplesToAnalyse.exprs$Amygdala),rank.method = "TDD",Nperm = 1000),msbb_gse84422.DCGs[c(1,10)],msbb_gse84422.DCLs[c(1,10)],mc.cores = mc)
msbb_array_DRrank.TDD[c(1:9,11:19)]=mcmapply(FUN=function(a,b)DRrank(DCGs = a,DCLs = b,tf2target = regnet_tf2target.HGNC,expGenes = rownames(msbb_gse84422_GPL96_97_samplesToAnalyse.exprs$Frontal_Pole),rank.method = "TDD",Nperm = 1000),msbb_gse84422.DCGs[c(1:9,11:19)],msbb_gse84422.DCLs[c(1:9,11:19)],mc.cores = mc)
stopCluster(cl)
saveRDS(msbb_array_DRrank.TDD,"msbb_array_DRrank_TDD.RDS")