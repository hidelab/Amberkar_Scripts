...
c_exprs_rank=msmm_data[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisSTG.CONTROL==1])]
t_exprs_rank=msmm_data[,which(colnames(msmm_data)%in%msmm_reseq_design_matrix$SampleID[msmm_reseq_design_matrix$BrainRegion.DiagnosisSTG.AD==1])]
n.c<-ncol(c_exprs_rank)
n.t<-ncol(t_exprs_rank)
gene.names<-rownames(msmm_data)
dir.create("results_STG",showWarnings = T,mode = "0777")
i<-0
blocksize=50
start<-i*blocksize+1
...
while(start < number_of_combinations){
  ...
  fwrite(result, file=paste("results_STG/mssm_reseq_STG_cocor_tmp", i, ".txt",sep = ""), sep="\t",col.names = T,row.names = F,buffMB = 100,nThread = 16,quote = F)
  i<-i+1
  start<-i*blocksize+1
  end<-min((i+1)*blocksize, number_of_combinations)
}
